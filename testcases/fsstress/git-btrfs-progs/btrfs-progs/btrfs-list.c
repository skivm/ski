			    ...
			    char **cache_full_name)
{
	...
	*/
	char tmp_str[256];
	sprintf(tmp_str, "inode %llu file offset %llu len %llu disk start %llu "
	       "offset %llu gen %llu flags ",
	       (unsigned long long)sh->objectid,
	       (unsigned long long)sh->offset,
	       (unsigned long long)len,
	...
	if (compressed) {
		sprintf(tmp_str + strlen(tmp_str), "COMPRESS");
		flags++;
	}
	if (type == BTRFS_FILE_EXTENT_PREALLOC) {
		sprintf(tmp_str + strlen(tmp_str), "%sPREALLOC", flags ? "|" : "");
		flags++;
	}
	if (type == BTRFS_FILE_EXTENT_INLINE) {
		sprintf(tmp_str + strlen(tmp_str), "%sINLINE", flags ? "|" : "");
		flags++;
	}
	if (!flags)
		sprintf(tmp_str + strlen(tmp_str), "NONE");
