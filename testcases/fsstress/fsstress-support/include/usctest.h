...
/* ================================================================  */
/* PF: SKI variables */
extern int STD_SKI_ENABLED, STD_SKI_WAIT_FOR_RESULTS, STD_SKI_CPU_AFFINITY, STD_SKI_HYPERCALLS, STD_SKI_USER_BARRIER, STD_SKI_TOTAL_CPUS, STD_SKI_TEST_NUMBER, STD_SKI_PROFILE_ENABLED;
extern int SKI_TEST_COUNTER;
void ski_test_start(int current_cpu, int total_cpus, int dry_run);
...
#define TEST(SCALL) \
	do { \
		...
		TEST_RETURN = 0; \
		TEST_ERRNO = errno; \
		if(STD_SKI_PROFILE_ENABLED){sleep(1);}\
		if(STD_SKI_ENABLED && (SKI_TEST_COUNTER==STD_SKI_TEST_NUMBER)){\
			ski_test_start(STD_SKI_CPU_AFFINITY,STD_SKI_TOTAL_CPUS,1);\
			ski_test_finish(STD_SKI_CPU_AFFINITY,1);\
			hypercall_debug(STD_SKI_CPU_AFFINITY, "Start test [TEST] - CPU: %d TCID: %s SKI_TEST_COUNTER: %d Tst_count: %d TST_TOTAL: %d SCALL: %s", \
				STD_SKI_CPU_AFFINITY, TCID, SKI_TEST_COUNTER, Tst_count, TST_TOTAL, #SCALL); \
		...
		if(STD_SKI_ENABLED && (SKI_TEST_COUNTER==STD_SKI_TEST_NUMBER)){\
			...
			hypercall_debug(STD_SKI_CPU_AFFINITY, "Stop test [TEST] - CPU: %d TCID: %s SKI_TEST_COUNTER: %d Tst_count: %d TST_TOTAL: %d SCALL: %s", \
				STD_SKI_CPU_AFFINITY, TCID, SKI_TEST_COUNTER, Tst_count, TST_TOTAL, #SCALL); \
			if(!STD_SKI_WAIT_FOR_RESULTS) {hypercall_debug(STD_SKI_CPU_AFFINITY, "END INFO");} \
		}\
		if(STD_SKI_PROFILE_ENABLED){sleep(2);}\
	} while (0)
...
#define TEST_VOID(SCALL) \
	do { \
		...
		if(STD_SKI_PROFILE_ENABLED){sleep(1);}\
		if(STD_SKI_ENABLED && (SKI_TEST_COUNTER==STD_SKI_TEST_NUMBER)){\
			hypercall_debug(STD_SKI_CPU_AFFINITY,"Start test [TEST_VOID] - CPU: %d TCID: %s SKI_TEST_COUNTER: %d Tst_count: %d TST_TOTAL: %d SCALL: %s", \
				STD_SKI_CPU_AFFINITY, TCID, SKI_TEST_COUNTER, Tst_count, TST_TOTAL, #SCALL); \
			ski_test_start(STD_SKI_CPU_AFFINITY, STD_SKI_TOTAL_CPUS, 1);\
			ski_test_finish(STD_SKI_CPU_AFFINITY, 1);\
			ski_test_start(STD_SKI_CPU_AFFINITY, STD_SKI_TOTAL_CPUS, 0);}\
		...
		if(STD_SKI_ENABLED && (SKI_TEST_COUNTER==STD_SKI_TEST_NUMBER)){\
			ski_test_finish(STD_SKI_CPU_AFFINITY, 0);\
			hypercall_debug(STD_SKI_CPU_AFFINITY,"Stop test [TEST_VOID] - CPU: %d TCID: %s SKI_TEST_COUNTER: %d Tst_count: %d TST_TOTAL: %d SCALL: %s", \
				STD_SKI_CPU_AFFINITY, TCID, SKI_TEST_COUNTER, Tst_count, TST_TOTAL, #SCALL); \
			if(!STD_SKI_WAIT_FOR_RESULTS) {hypercall_debug(STD_SKI_CPU_AFFINITY, "END INFO");} \
		}\
		if(STD_SKI_PROFILE_ENABLED){sleep(2);}\
