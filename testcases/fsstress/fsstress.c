...
#ifdef BTRFS_ENABLED
#warning "=============== Using POSIX ops + BTRFS ops =============== "
#define BTRFS_SUBVOL 
#define BTRFS_FS 
#define BTRFS_DEV

#else	//BTRFS_ENABLED
#warning "=============== Using POSIX ops only =============== "
...
typedef enum {
	...
	OP_WRITE,
#ifdef BTRFS_SUBVOL
	OP_BTRFS_SUBVOL_SNAPSHOT,
	OP_BTRFS_SUBVOL_DELETE,
	OP_BTRFS_SUBVOL_CREATE,
	OP_BTRFS_SUBVOL_LIST,
	OP_BTRFS_SUBVOL_FINDNEW,
#endif
#ifdef BTRFS_FS
	OP_BTRFS_FS_DEFRAGMENT,
	OP_BTRFS_FS_SYNC,
	OP_BTRFS_FS_RESIZE,
	OP_BTRFS_FS_SHOW,
	OP_BTRFS_FS_DF,
...
#define	FT_NOTVOL	(FT_ANYm & ~FT_VOLm)
...
void btrfs_ioctl_subvolume_snapshot(int opno, long r);
void btrfs_ioctl_subvolume_delete(int opno, long r);
void btrfs_ioctl_subvolume_create(int opno, long r);
void btrfs_ioctl_subvolume_list(int opno, long r);
void btrfs_ioctl_subvolume_findnew(int opno, long r);

void btrfs_ioctl_filesystem_defragment(int opno, long r);
void btrfs_ioctl_filesystem_sync(int opno, long r);
void btrfs_ioctl_filesystem_resize(int opno, long r);
void btrfs_ioctl_filesystem_show(int opno, long r);
void btrfs_ioctl_filesystem_df(int opno, long r);
void btrfs_ioctl_filesystem_balance(int opno, long r);

void btrfs_ioctl_device_add(int opno, long r);
void btrfs_ioctl_device_delete(int opno, long r);

char ski_testdir[256];

char* ski_secondary_device = "/dev/loop1";

#define SKI_FSSTRESS_REVISION "$Rev: 2332 $"
...
#ifdef BTRFS_SUBVOL
    {OP_BTRFS_SUBVOL_SNAPSHOT, "btrfs_subvol_snapshot", btrfs_ioctl_subvolume_snapshot, 1, 1}, 
    {OP_BTRFS_SUBVOL_DELETE, "btrfs_subvol_delete", btrfs_ioctl_subvolume_delete, 1, 1},
    {OP_BTRFS_SUBVOL_CREATE, "btrfs_subvol_create", btrfs_ioctl_subvolume_create, 2, 1},
    {OP_BTRFS_SUBVOL_LIST, "btrfs_subvol_list", btrfs_ioctl_subvolume_list, 1, 1},
    {OP_BTRFS_SUBVOL_FINDNEW, "btrfs_subvol_findnew", btrfs_ioctl_subvolume_findnew, 1, 1},
#endif
#ifdef BTRFS_FS
	{OP_BTRFS_FS_DEFRAGMENT, "btrfs_ioctl_filesystem_defragment", btrfs_ioctl_filesystem_defragment, 1 , 1},
	{OP_BTRFS_FS_SYNC, "btrfs_ioctl_filesystem_sync", btrfs_ioctl_filesystem_sync, 1, 1},
	{OP_BTRFS_FS_RESIZE, "btrfs_ioctl_filesystem_resize", btrfs_ioctl_filesystem_resize, 1, 1},
	{OP_BTRFS_FS_SHOW, "btrfs_ioctl_filesystem_show", btrfs_ioctl_filesystem_show, 1, 1},
	{OP_BTRFS_FS_DF, "btrfs_ioctl_filesystem_df", btrfs_ioctl_filesystem_df, 1, 1},
	{OP_BTRFS_FS_BALANCE, "btrfs_ioctl_filesystem_balance", btrfs_ioctl_filesystem_balance, 1, 1},
#endif
#ifdef BTRFS_DEV
	{OP_BTRFS_DEV_ADD, "btrfs_ioctl_device_add", btrfs_ioctl_device_add, 1, 1},
	{OP_BTRFS_DEV_DELETE, "btrfs_ioctl_device_delete", btrfs_ioctl_device_delete, 1, 1},
#endif 
}, *ops_end;
...
/* PF: need to make sure this is correct */
void del_from_flist_specific_id(int ft, int id)
{
	flist_t *ftp;
	...
	int slot = -1;
	int i;
	for(i=0;i<flist[ft].nfiles;i++){
		if(flist[ft].fents[i].id == id){
			slot = i;
			break;
		}
	}
	if(slot == -1){
		printf("ERROR: del_from_flist_specific_id unable to find the specific file (ft = %d, id = %d)\n", ft, id);
		return;
	}
	printf("del_from_flist_specific_id found the file (slot = %d)\n", slot);
...
void doproc(void)
{
	...
	// Dry run (to avoid page faults)
	ski_test_start(STD_SKI_CPU_AFFINITY, STD_SKI_TOTAL_CPUS, 1);

	int initial_state_operations = 1000;
	int initial_state_seed = 0;
	//int initial_state_seed = time(0);

	hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*)"Initial state test [TEST] - CPU: %d Initial_state_operations: %d Seed: %d",
			STD_SKI_CPU_AFFINITY, initial_state_operations, initial_state_seed);

	srandom(initial_state_seed);

	/* Spark: Create the initial state */
	for (opno = 0; opno < initial_state_operations; opno++) {

		while(1){
			int op_index = freq_table[random() % freq_table_size];
			p = &ops[op_index];
	
			//hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*)"Random op_index = %d\n", op_index);
			if((op_index > OP_WRITE) && ((random() % 20) != 0)){
				// Reduce the probability of selecting a btrfs op during the initial phase
				// Select again another random op
				continue;
			}
			if((p->func == btrfs_ioctl_filesystem_balance) && ((initial_state_operations - opno) > 100)){
				// Avoid balance op because it's very very slow....
				continue;
			}
			if((p->func == btrfs_ioctl_device_add)){
				// Don't add devices in the initial operations
				continue;
			}

			if(((p->func == mkdir_f) || (p->func == rmdir_f)) && ((random() % 10 )!= 0)){ 
				// Reduce the frequency of mkdir and rmdir calls 
				// Select again another random op
				continue;	
			}
		...
		int op_seed = random();
		hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*)"Initial state test [TEST] - CPU: %d Op: %s Op_seed: %d Op_no: %d Op_max: %d",
				STD_SKI_CPU_AFFINITY, p->name, op_seed, opno, initial_state_operations);
...
#ifdef BTRFS_ENABLED
	hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*)"BTRFS test");
#else   
	hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*)"POSIX test");
#endif 

	hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*)"First call");

	int child_pid = 0;
	if(STD_SKI_FORK_ENABLED){
		if((child_pid = fork())){
			// Parent
			hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*)"Parent process (FORK enabled)");
		}else{
			// Child
			STD_SKI_CPU_AFFINITY += 1;
			hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*)"Child process (FORK enabled)");
		}
	}


    int mlock_parameters = MCL_CURRENT; //| MCL_FUTURE;
    if(mlockall(mlock_parameters) == -1) {
	...
	if(ret == 1){
		// ret == 1, by convention used during the snapshot creation so no need to run operations
		operations = 0;
	}else{
		//operations = (random() % 6) + 5;
		operations = (random() % 2) + 1;
	}
	...
	for (opno = 0; opno < operations; opno++) {
		...
		hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*)"Start test [TEST] - CPU: %d Op: %s Op_seed: %d Op_no: %d Op_max: %d Test_seed: %d ",
				STD_SKI_CPU_AFFINITY, p->name, op_seed, opno, operations, test_seed);
	...
	// For historic reasons the current folder is one level deeper than the scripts...
	...
	if(STD_SKI_FORK_ENABLED){
		if(child_pid){
			// Parent
			hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*)"Parent waiting (FORK enabled)");
			int ret;
			int pid_wait = wait(&ret);
			hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*)"Parent finished waiting, ret = %d, pid_wait = %d (FORK enabled)", ret, pid_wait);

		}else{
			hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*)"Child exiting (FORK enabled)");
			// Child
			exit(0);
		}
	}
	...
	if(STD_SKI_CPU_AFFINITY == 0){
		sleep(0);
		hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*)"Fsstress version: %s", SKI_FSSTRESS_REVISION);
		system("/root/fsstress-ski/check.sh"); 
...
void move_specific_subvolume(int oldid, int newid, int newparent)
{
	fent_t *fep;
	flist_t *flp;
	int i;
	...
	for(i=0 ; i < flp->nfiles ; i++){
		...
		if(fep->id == oldid){
			fep->id = newid;
			fep->parent = newparent;
			printf("move_specific_subvolume: success (oldid = %d, newid = %d, newparent = %d)\n", oldid, newid, newparent);
			return;
		}
	}
	printf("move_specific_subvolume: failed (oldid = %d, newid = %d, newparent = %d)\n", oldid, newid, newparent);
}
	  ...
	  int *v)
{
		...
		if (which & (1 << i)) {
			if (x < c + flp->nfiles) {
				...
				*v = verbose;
				...
				if(name){
					printf("CPU: %d get_fname() - name: %s\n", STD_SKI_CPU_AFFINITY, name->path);
				}
				return 1;
			}
			c += flp->nfiles;
	...
	printf("CPU: %d get_fname() - failed (ERROR)\n", STD_SKI_CPU_AFFINITY);
#ifdef DEBUG
...
int open_path(pathname_t * name, int oflag)
{
	...
	//hypercall_debug_quiet(STD_SKI_CPU_AFFINITY, (char*) "Opened file \"%s\" (flags: 0%o ret: %d)", name->path, oflag, rval);
...
void rename_f(int opno, long r)
{
	...
	init_pathname(&f);
	if (!get_fname(FT_NOTVOL, r, &f, &flp, &fep, &v1)) {
	...
	if (e == 0) {
		if (flp - flist == FT_DIR) {
			oldid = fep->id;
			fix_parent(oldid, id);
			move_specific_subvolume(oldid, id, parid);
		}
...
*/
    ...
    printf("btrfs_ioctl_subvolume_snapshot\n");

    init_pathname(&f);
    if (!get_fname(FT_VOLm, r, &f, &flp, &fep, &v1)) {
        if (v1)
            printf("%d/%d: btrfs_ioctl_subvolume_snapshot - no filename\n", procid, opno);
        free_pathname(&f);
        return;
    }
    if (!get_fname(FT_DIRm, random(), NULL, NULL, &dfep, &v))
    ...
    v |= v1;
    init_pathname(&newf);
    e = generate_fname(dfep, FT_DIR, &newf, &id, &v1);
    v |= v1;
    if (!e) {
        if (v) {
            fent_to_name(&f, &flist[FT_DIR], dfep);
            printf("%d/%d: btrfs_ioctl_subvolume_snapshot - no filename from %s\n",
                   procid, opno, f.path);
        }
        free_pathname(&newf);
        free_pathname(&f);
    ...
    argv[3] = newf.path;
    argv[4] = 0;
    e = cmd_subvolume(argc, argv);

    check_cwd();

	// XXX: We're not creating the entries for the new files that are copied to the snapshot because they existed in the original subvolume
      // In theory we should do this (otherwise not writes/read of the files that were snapshoted)
    if (e == 0) {
		/*
        if (flp - flist == FT_DIR) {
            oldid = fep->id;
    ...
    if (v)
        printf("%d/%d: btrfs_ioctl_subvolume_snapshot %s to %s %d\n", procid, opno, f.path,
               newf.path, e);
    free_pathname(&newf);
    free_pathname(&f);

	//printf("%d/%d: NYI: btrfs_ioctl_subvolume_snapshot\n", procid, opno);
...
*/
    ...
    init_pathname(&f);
    if (!get_fname(FT_VOLm, r, &f, NULL, &fep, &v)) {
        if (v)
            printf("%d/%d: btrfs_ioctl_subvolume_delete - no directory\n", procid, opno);
        free_pathname(&f);
        return;
    }
    ...
    check_cwd();
    if (e == 0){
		del_from_flist_specific_id(FT_DIR, fep->id);
        del_from_flist(FT_VOL, fep - flist[FT_VOL].fents);
	}
    if (v)
        printf("%d/%d: btrfs_ioctl_subvolume_delete %s %d\n", procid, opno, f.path, e);
    free_pathname(&f);
...
*/
	...
	/* Most of this code comes from the mkdir_f */
	int e;
	pathname_t f;
	fent_t *fep;
	int id;
	...
	if (!get_fname(FT_DIRm, r, NULL, NULL, &fep, &v))
		...
		parid = fep->id;
	init_pathname(&f);
	e = generate_fname(fep, FT_VOL, &f, &id, &v1);
	v |= v1;
	if (!e) {
		if (v) {
			fent_to_name(&f, &flist[FT_DIR], fep);
			printf("%d/%d: btrfs_ioctl_subvolume_create - no filename from %s\n",
			       procid, opno, f.path);
		}
		free_pathname(&f);
		return;
	...
	if (e == 0){
		add_to_flist(FT_DIR, id, parid);
		add_to_flist(FT_VOL, id, parid);
	}
	if (v)
		printf("%d/%d: btrfs_ioctl_subvolume_create %s %d\n", procid, opno, f.path, e);
	free_pathname(&f);
...
void btrfs_ioctl_subvolume_list(int opno, long r)
{
    ...
    init_pathname(&f);
    if (!get_fname(FT_VOLm, r, &f, NULL, NULL, &v)) {
        if (v)
            printf("%d/%d: btrfs_ioctl_subvolume_list - no entries\n", procid, opno);
        free_pathname(&f);
        return;
    }
    //e = lstat64_path(&f, &stb) < 0 ? errno : 0;
    ...
    check_cwd();
    if (v)
        printf("%d/%d: btrfs_ioctl_subvolume_list %s %d\n", procid, opno, f.path, e);
    free_pathname(&f);
}
...
void btrfs_ioctl_subvolume_findnew(int opno, long r)
{
    ...
    if (!get_fname(FT_VOLm, r, &f, NULL, NULL, &v)) {
        if (v)
            printf("%d/%d: btrfs_ioctl_subvolume_findnew - no entries\n", procid, opno);
        free_pathname(&f);
        return;
    }   
    //e = lstat64_path(&f, &stb) < 0 ? errno : 0;
    ...
    check_cwd();
    if (v)
        printf("%d/%d: btrfs_ioctl_subvolume_findnew %s %d\n", procid, opno, f.path, e);
    free_pathname(&f);
}
...
void btrfs_ioctl_filesystem_defragment(int opno, long r)
{
    ...
    if (!get_fname(f_type, r, &f, NULL, NULL, &v)) {
        if (v)
            printf("%d/%d: btrfs_ioctl_filesystem_defragment - no entries\n", procid, opno);
        free_pathname(&f);
        return;
    }   
    int argc = 3;
    ...
    argv[2] = f.path;
    argv[3] = 0;
    e = cmd_filesystem(argc, argv);

    check_cwd();
    if (v)
        printf("%d/%d: btrfs_ioctl_filesystem_defragment %s %d\n", procid, opno, f.path, e);
    free_pathname(&f);
}
...
void btrfs_ioctl_filesystem_sync(int opno, long r)
{
    ...
    check_cwd();
    if (v)
        printf("%d/%d: btrfs_ioctl_filesystem_sync %d\n", procid, opno,  e);
...
void btrfs_ioctl_filesystem_resize(int opno, long r)
{
	...
	int increase=(random()%2);
	int size = (random() % 10) * 1 + 1; // In MB

	assert(ski_testdir[0]);

	sprintf(size_str, "%c%dm", increase?'+':'-', size);
    int argc = 4;
    char *argv[10];
	argv[0] = "filesystem";
    argv[1] = "resize";
    ...
    check_cwd();
    if (v)
        printf("%d/%d: btrfs_ioctl_filesystem_resize %s %s %d\n", procid, opno, size_str, ski_testdir, e);
}
...
void btrfs_ioctl_filesystem_show(int opno, long r)
{
    ...
    check_cwd();
    if (v)
        printf("%d/%d: btrfs_ioctl_filesystem_show %d\n", procid, opno, e);
...
void btrfs_ioctl_filesystem_df(int opno, long r)
{
    ...
    if (!get_fname(f_type, r, &f, NULL, NULL, &v)) {
        if (v)
            printf("%d/%d: btrfs_ioctl_filesystem_df - no entries\n", procid, opno);
        free_pathname(&f);
        return;
    }   
    int argc = 3;
    ...
    check_cwd();
    if (v)
        printf("%d/%d: btrfs_ioctl_filesystem_df %s %d\n", procid, opno, f.path, e);
    free_pathname(&f);
...
void btrfs_ioctl_filesystem_balance(int opno, long r)
{
    ...
    if (!get_fname(f_type, r, &f, NULL, NULL, &v)) {
        if (v)
            printf("%d/%d: btrfs_ioctl_filesystem_balance - no entries\n", procid, opno);
        free_pathname(&f);
        return;
    }   
    int argc = 3;
    ...
    check_cwd();
    if (v)
        printf("%d/%d: btrfs_ioctl_filesystem_balance %s %d\n", procid, opno, f.path, e);
    free_pathname(&f);
}
...
void btrfs_ioctl_device_add(int opno, long r)
{
    ...
    argv[0] = "device";
    argv[1] = "add";
    argv[2] = ski_secondary_device;
    argv[3] = ski_testdir;
    e = cmd_device(argc, argv);

    check_cwd();
    if (v)
        printf("%d/%d: btrfs_ioctl_device_add %s %s %d\n", procid, opno, ski_secondary_device, ski_testdir, e);
}
...
void btrfs_ioctl_device_delete(int opno, long r)
{
    ...
    check_cwd();
    if (v)
        printf("%d/%d: btrfs_ioctl_device_delete %s %s %d\n", procid, opno, ski_secondary_device, ski_testdir, e);
}
