...
//    env->exception_index = -1;
	...
	if((ski_init_options_input_number[env->cpu_index])>-1){
		hio.p.hio_test_enter.hg_res = ski_init_options_input_number[env->cpu_index];
	}else{
...
void ski_threads_invert_priority(CPUState * env)
{
	...
	SKI_TRACE("ski_threads_invert_priority: Preemption point (EIP: %x) - changing priority of thread %d from %d to %d\n", env->eip, tc->id, tc->priority, new_priority);
	sprintf(debug_str, "Preemption point (EIP: %x)", env->eip);
	ski_exec_trace_print_comment(debug_str);	
		...
		if(tc==0 || tc->cpu_no == env->cpu_index){
			...
			if(tc && (ski_threads_current != tc)){
				...
				sprintf(trace_comment,"Switching to thread %d, CPU %d [%d, %d] from thread %d, CPU %d [%d, %d]", 
					tc->id, tc->cpu_no, tc->priority, tc->priority_adjust,
