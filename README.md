...
- SKI executes tests in (nearly-)deterministically
- SKI supports several types of bug detectors (data races, panics, asserts, warnings, ...)

The level of control provided by SKI and its heuristics increase the probability and speed of exposing 
concurrency bugs. Additionally, the control achieved by SKI's together with its tracing capability 
allows developers to easily compare different execution paths caused by different interleavings 
(which can be important to diagnose the more complex concurrency bugs).
...
SKI is based on QEMU 1.0.1 and has the same building dependencies (see `vmm/README` for QEMU's original 
...
given the current scripts and samples of test cases the VM image should:
 ...
 - Report using hypercalls when the machine finishes booting. This allows `create-snapshot.sh` to know when to upload the test case. 
 - Have network access configured and root SSH access enabled with the keys in the SKI package. This is allows `create-snapshot.sh` to automatically upload the test case.
...
procedures (e.g., formatting a file system), the test cases are expected to perform the following tasks:
 ...
 3. Each of the threads or processes drives the kernel by issuing system calls. (The value returned by `ski_test_start()` is usually leveraged to control the system calls selected and/or its parameters, i.e., the returned value is interpreted as an testing input specifier.) 
...
Running the tests requires the following configuration steps to be performed:
 ...
 - `SKI_TMP` should point to a temporary location with space for at least a few GB (preferably on a tmpfs for 
   faster accesses)
 - Ensure that the shared segment limits are sufficiently high. For example:  
